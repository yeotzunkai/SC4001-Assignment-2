...
test_data_dir = 'splitted_dataset/valid'
...
state_dict = torch.load('inception_flower_model2.pth', map_location=device)
...
## The whole below to run whole folder accuracy results.
...
'''
# The whole below section to RUN random image results
...
## Run BELOW for single image inference
'''
# Function to perform inference on a single image
def predict_image(image_path, model):
    image = Image.open(image_path)
    image_transformed = inference_transforms(image).unsqueeze(0)
...
# Perform prediction
predicted_label = predict_image(image_path, inception)
